...
\code{backend.view} befinden, diese sind jedoch 
...
nach dem Programmstart durch den Aufruf der Methode \code{open()} mit einer \gls{ip}-Adresse und einem Port initialisiert und kann anschließend 
darüber Nachrichten (Anfragen für den Aufbau einer \gls{tcp}-Verbindung) von den Devices entgegennehmen. \\
Die Abbildung \eqref{fig:opensocket} zeigt die Funktion \code{open()}.
\begin{figure}[ht]
	\centering
...
Die Abbildung \eqref{fig:listen} zeigt die Methode \code{listen()} in der ankommende Verbindungsaufbauwünsche verarbeitet werden.
\begin{figure}[ht]
	\centering
...
\begin{enumerate}
	...
	%   ###############################################################################################################################
\end{enumerate}
Der erste Schritt wird durch die Methode \code{public void parse(String pCommand)} realisiert. Zur Umwandlung des vorliegenden Strings verwendet
...
\newpage
\noindent
Die Abbildung \eqref{fig:parse} zeigt den Quellcode des Parse-Vorgangs in der Methode Vorgangs \code{public void parse(String pCommand)}.
\begin{figure}[ht]
	\centering
...
\begin{itemize}
	...
	%   ###############################################################################################################################
	\item{\code{private void connection(JSONObject pCommand)}} -- Verarbeitet ein als \gls{json}-Objekt vorliegendes Datenpaket vom Typ 
	Kommandotyp Connection.
	%   ###############################################################################################################################
	\item{\code{private void synchronization(JSONObject pCommand)}} -- Verarbeitet ein als \gls{json}-Objekt vorliegendes Datenpaket vom 
	Typ Kommandotyp Synchronization.
	%   ###############################################################################################################################
	\item{\code{private void szenario(JSONObject pCommand)}} -- Verarbeitet ein als \gls{json}-Objekt vorliegendes Datenpaket vom Typ 
	...
	\end{itemize}
	%   ###############################################################################################################################
	\item{\code{private void exception(JSONObject pCommand)}} -- Verarbeitet ein als \gls{json}-Objekt vorliegendes Datenpaket vom Typ 
...
\begin{itemize}
	...
	%   ###############################################################################################################################
	\item{\code{float getRealSpeed()}} -- Soll die tatsächliche (nicht eingestellte) Geschwindigkeit des Roboters zurückgeben.
	%   ###############################################################################################################################
	\item{\code{Robot getJSONRobot()}} -- Soll alle wichtigen Roboterdaten in einem speziellen Roboter-Objekt zurückgeben, welches 
...
\begin{wrapfigure}{r}{0.30\textwidth}
	...
	\label{fig:dreirad}
\end{wrapfigure}
\paragraph{Die ThreeWheelDrive Klasse}             % Farbe die angibt welchen Status der folgende Abschnitt hat!
% Eigentlicher Text:
...
\begin{itemize}
	%   ###############################################################################################################################
	\item{\code{private EV3MediumRegulatedMotor motorRight, motorLeft;}} -- Das EV3\-Me\-dium\-RegulatedMotor-Objekt der EV3-Java-Bibliothek
...
\begin{itemize}
	%   ###############################################################################################################################
	\item{\code{public static final float DISTANCE\_DEGREE =   0.476389f;}} -- Dieser Parameter gibt die Distanz in Millimetern 
...
werden.
\subsubsection{Steuerung}\label{robotsteuerung}
...
\end{figure}
\newline
Die Abbildung \eqref{fig:controlRobotMethode} zeigt einen Ausschnitt der Methode \code{controlRobot()} innerhalb der RobotController-Klasse. Der Quellcode realisiert die direkte Steuerung des Roboters durch die beim Roboter eintreffenden Steuersignale der App. Die 
...
Anschließend wird zuerst die im Streukommando codierte Richtungsangabe verarbeitet, indem je nach Angabe, der Roboter entweder durch Aufruf der Funktion \code{rotate(direction)} veranlasst wird eine Drehung in die entsprechende Richtung durchzuführen oder sich geradeaus zu bewegen (\code{forward()}).
\\
Nach Verarbeitung der Richtungsangabe wird die Geschwindigkeitsangabe verarbeite. Soll der Roboter hierbei schneller Fahren, wird die Methode \code{increaseSpeed()} aufgerufen, zur Verlangsamung wird die Methode \code{decreaseSpeed()} aktiviert. Soll die Geschwindigkeit konstant bleiben, wird keine der beiden Methoden aufgerufen.
Zum Abschluss wird das aktuelle Steuerkommando auf den Wert null gesetzt, um zu markieren, dass dieses Verarbeitet wurde.
\medskip
\newline
Wie die jeweiligen Funktionen \code{rotate(direction)}, \code{forward()} etc. konkret realisiert sind, liegt in der Verantwortung der jeweiligen Roboter-Klasse und ist abhängig von seinem physischen Aufbau.
\medskip
\newline
...
\subsubsection{Datenerfassung \& Synchronisation}\label{robotdatenerfassung}
...
\code{synchronize()}-Methode innerhalb der RoboterController-Klassen.
Abbildung \eqref{fig:synchronizeRobot} zeigt den Quellcode der \code{synchronize()}-Methode.
\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{images/implementation/synchronizeMethod.png}
	\caption{\code{synchronize()}-Methode der RobotController-Klasse im Robot}
...
Datenobjekt zurückgeliefert. Anschließend wird dieses Datenobjekt konvertiert und durch Aufruf der Funktion \code{sendCmd(gson.toJson(sync))} an das Backend gesendet. Nach abwarten einer 
konfigurierbaren Zeit startet dieser Prozess von neuem.
\newline
\newpage
\noindent
Die Abbildung \eqref{fig:getJSONRobot} zeigt die Implementierung der Funktion {\code{getJSONRobot()} in der Klasse ThreeWheelDrive, welche das Datenobjekt mit den 
entsprechenden Roboterdaten erzeugt.
\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{images/implementation/getJSONRobotMethod.png}
	\caption{\code{getJSONRobot()}-Methode der ThreeWheelDrive-Klasse im Robot}
	\label{fig:getJSONRobot}
\end{figure}
\newline
Zur eigentlichen Erfassung der Daten von den Robotersensoren und deren Aufbereitung nutzt die Roboterklasse mehrere Methoden, wie \code{getPosition()} und \code{getRealSpeed()}.
Abbildung \eqref{fig:getPosition} zeigt exemplarisch, wie beim ThreeWheelDrive-Roboter die Position und Orientierung durch die Methode \code{getPosition()} berechnet und
...
\begin{figure}[ht]
	...
	\caption{\code{getPosition()}-Methode der ThreeWheelDrive-Klasse im Robot}
	\label{fig:getPosition}
\end{figure}
\newline
Zur letztendlichen Berechnung der aktuellen Position und Orientierung auf Grundlage der ausgelesenen und verarbeiteten Sensordaten nutzt die \code{getPosition()}-Methode standardisierte Methoden der Position-Klassen, welche eine Position inklusive Orientierung repräsentiert.